...
class Budget extends Model
{
    ...
    static function getBudgetTableByYear($year, $month = 12)
    {
        ...
        $brief = new stdClass();
        $brief->year = $year;
        $brief->income = Budget::where('type', 'Income')->whereRaw('extract(year from created_at) = ?', [$year])->where('user_id', auth()->user()->id)->sum('amount');
        $brief->expense = Budget::where('type', 'Expense')->whereRaw('extract(year from created_at) = ?', [$year])->where('user_id', auth()->user()->id)->sum('amount');
        $brief->net_budget = Budget::whereRaw('extract(year from created_at) = ?', [$year])->where('user_id', auth()->user()->id)->sum('amount');
